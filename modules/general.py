...
class General(commands.Cog):
    ...
    async def build(self, ctx): # p!build
        ...
        await ctx.reply("Here's some information about me!", embed = embed)
    ...
    async def invite(self, ctx, server = None): # p!invite
        ...
        if server == None or code == None:
            server = "Nincord"; code = arrays.INVITECODES.get(server.capitalize())
        if server.capitalize() == "Nincord":
            await ctx.reply(f"Share this link to invite people to our server! https://discord.gg/{code}")
    ...
    async def userinfo(self, ctx, user: typing.Union[discord.Member, discord.User, int, str] = None): # p!userinfo
        ...
        try:
            if isinstance(user, (discord.User, int)):
                ...
                user = await self.bot.fetch_user(user[3:-1])

            async def returning_user_information(user):
                ...
                embed.add_field(name = "Created", value = "{}".format(format_create), inline = False)
                if isinstance(user, discord.Member):
                    embed.add_field(name = "Joined", value = "{}".format(format_join), inline = False)
                ...
                if not check_staff_member(user):
                    if collection.count_documents({"_id": user.id}, limit = 1) != 0:
                        ...
                        if check_warn_3 != None:
                            ...
                            warns = "2"
                        elif check_warn_2 == None and check_warn_1 != None:
                            warns = "1"
                ...
                if user == ctx.author:
                    await ctx.reply("Here's some information about you.", embed = embed)
                else:
                    await ctx.reply(f"Here's some information about {user.mention}.", embed = embed)

            if check_staff_member(ctx.author):
                if user == None: user = ctx.author
                return await returning_user_information(user)

            if user != ctx.author and user != None:
                return await ctx.reply("Using this command on others is only allowed for staff members.")
            user = ctx.author; return await returning_user_information(user)
    ...
    async def serverinfo(self, ctx): # p!serverinfo
        ...
        embed.add_field(name = "Created", value = format_create, inline = False)
        ...
        await ctx.reply("Here's some information about the server.", embed = embed)
    ...
    async def roletoggle(self, ctx, role): # p!roletoggle
        ...
        await ctx.reply("This is not a valid togglable role.")
    ...
    async def suggest(self, ctx, *, idea): # p!suggest
        ...
        embed.add_field(name = "Suggestion", value = idea, inline = False)
        ...
        await ctx.reply("Your suggestion has successfully been submitted. It will be reviewed internally.")
