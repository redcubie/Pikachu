...
class Moderation(commands.Cog):
    ...
    async def warn_error(self, ctx, error): # p!warn error handlers.
        if isinstance(error, commands.BadArgument):
            return await ctx.send(f"{ctx.author.mention}, this user cannot be found. Please specify the correct user.")
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f"{ctx.author.mention}, there is no user or reason specified. Please specify a user and reason.")
    ...
    async def pardon_error(self, ctx, error): # p!pardon error handlers.
        if isinstance(error, commands.BadArgument):
            return await ctx.send(f"{ctx.author.mention}, this user cannot be found. Please specify the correct user.")
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f"{ctx.author.mention}, there is no user specified. Please specify a user.")
    ...
    async def slowmode_error(self, ctx, error): # p!slowmode error handlers.
        if isinstance(error, commands.BadArgument):
            return await ctx.send(f"{ctx.author.mention}, this time is invalid. Please specify the correct amount of time.")
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f"{ctx.author.mention}, there is no time specified. Please specify an amount of time.")

    @lockmode.error
    async def lockmode_error(self, ctx, error): # p!lockmode error handlers.
        if isinstance(error, commands.BadArgument):
            return await ctx.send(f"{ctx.author.mention}, this channel is invalid. Please specify the correct channel.")
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f"{ctx.author.mention}, there is no channel specified. Please specify a channel.")

    @filters.error
    async def filters_error(self, ctx, error): # p!filters error handlers.
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(f"{ctx.author.mention}, there is no action specified. Please specify an action.")
