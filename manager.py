...
bot = commands.Bot(command_prefix = ("P!", "p!"), case_insensitive = True, allowed_mentions =
    discord.AllowedMentions.none(), intents=intents)
...
async def on_message(ctx):
    if not isinstance(ctx.channel, discord.channel.DMChannel) and ctx.content.lower().startswith("p!"):
        try:
            ...
            if ctx.channel.id in arrays.CHANNELINFORMATION:
                ...
                bot_setting = info.get("Bot")
                if bot_setting: return await bot.process_commands(ctx)
                else: return None
...
async def load(ctx, extension): # p!load
    ...
    try:
        ...
        await ctx.reply(f"The {extension} module is already loaded.")
    except commands.ExtensionNotFound:
        await ctx.reply(f"The {extension} module does not exist.")
...
async def unload(ctx, extension): # p!unload
    ...
    try:
        ...
        await ctx.reply(f"The {extension} module is already unloaded.")
    except commands.ExtensionNotFound:
        await ctx.reply(f"The {extension} module does not exist.")
...
@commands.has_permissions(administrator = True)
async def dictionary(ctx, dictionary, search, attribute = None, setting: bool = None): # p!key
    ...
    if dictionary.lower() == "role" or dictionary.upper() == "ROLEINFORMATION":
        ...
        try:
            search = getattr(variables, search.upper())
            result = arrays.ROLEINFORMATION.get(search)
        except: result = None
        ...
        try:
            search = getattr(variables, search.upper())
            result = arrays.CHANNELINFORMATION.get(search)
        except: result = None
    else: return await ctx.reply("The dictionary you have specified is not a valid option.")
    if attribute != None:
    ...
    if setting != None:
        if dictionary == "ROLEINFORMATION": result = arrays.ROLEINFORMATION.get(search)
        elif dictionary == "CHANNELINFORMATION": result = arrays.CHANNELINFORMATION.get(search)
        if not isinstance(result[attribute], bool):
            return await ctx.reply("This value tied to the role cannot be edited.")
    ...
    if result == None:
        await ctx.reply(f"Here are the keys and values for your specified dictionary.\n```Input: {search}\nOutput: None```")
    elif attribute == None:
        await ctx.reply(f"Here are the keys and values for your specified dictionary.\n```Input: {search}\nOutput: {result}```")
    else: 
